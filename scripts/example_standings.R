...
main <- function() {
  ...
  tryCatch({
    ...
    standings_data <- expand.grid(
      ...
      mutate(
        ...
        race_points = case_when(
          driver == "VER" ~ sample(c(25, 18, 15), size = n(), replace = TRUE, prob = c(0.6, 0.3, 0.1)),
          driver == "HAM" ~ sample(c(25, 18, 15, 12, 10), size = n(), replace = TRUE, prob = c(0.1, 0.3, 0.3, 0.2, 0.1)),
          driver == "LEC" ~ sample(c(25, 18, 15, 12, 10), size = n(), replace = TRUE, prob = c(0.15, 0.25, 0.3, 0.2, 0.1)),
          TRUE ~ sample(c(15, 12, 10, 8, 6, 4, 2, 1, 0), size = n(), replace = TRUE)
        ),
    ...
    # Create plot
    p <- ggplot(plot_data, aes(x = race, y = total_points, color = driver, group = driver)) +
      ...
      labs(
        title = sprintf("%d F1 Season - Championship Standings Progression", SEASON),
    ...
    output_file <- file.path(OUTPUT_DIR, sprintf("standings_progression_%d.png", SEASON))
