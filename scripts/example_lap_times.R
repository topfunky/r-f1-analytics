...
main <- function() {
  ...
  cache_file <- file.path(CACHE_DIR, sprintf("race_info_%d_%d.rds", SEASON, RACE))
  ...
  tryCatch({
    ...
    lap_data <- expand.grid(
      ...
      mutate(
        lap_time = 90 + rnorm(n(), mean = 0, sd = 2) + 
    ...
    # Create plot
    p <- ggplot(lap_data, aes(x = lap, y = lap_time, color = driver, group = driver)) +
      ...
      labs(
        title = sprintf("%d Season - Race %d: Lap Time Analysis", SEASON, RACE),
    ...
    output_file <- file.path(OUTPUT_DIR, sprintf("lap_times_%d_race%d.png", SEASON, RACE))
