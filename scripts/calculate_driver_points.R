#!/usr/bin/env Rscript
# Calculate individual driver points using post-2010 scoring system
...
# Function to fetch race results for a specific season and round with caching
fetch_race_results <- function(season, round) {
...
# Function to get all races for a season
...
# Function to get driver names
get_driver_names <- function(season) {
...
# Function to get constructor names
get_constructor_names <- function(season) {
...
# Main function to calculate driver points for a season
calculate_season_points <- function(season) {
  ...
  driver_names <- get_driver_names(season)
  constructor_names <- get_constructor_names(season)
...
main <- function() {
  ...
  for (season in START_YEAR:END_YEAR) {
    ...
    if (!is.null(season_race_points)) {
      ...
      cat(sprintf("  ✓ Processed %d races for season %d\n", 
                  length(unique(season_race_points$round)), season))
    } else {
