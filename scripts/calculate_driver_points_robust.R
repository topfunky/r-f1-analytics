...
fetch_race_results_with_retry <- function(
  ...
  for (attempt in 1:max_retries) {
    ...
    if (is.null(results) || !is.data.frame(results) || nrow(results) == 0) {
      ...
      cat(sprintf(
        "  Attempt %d failed for race %d, retrying in %d seconds...\n",
        attempt,
        round,
        RETRY_DELAY
      ))
...
get_season_races_with_retry <- function(season, max_retries = MAX_RETRIES) {
  ...
  for (attempt in 1:max_retries) {
    ...
    if (is.null(schedule) || !is.data.frame(schedule) || nrow(schedule) == 0) {
      ...
      cat(sprintf(
        "  Attempt %d failed for season %d schedule, retrying in %d seconds...\n",
        attempt,
        season,
        RETRY_DELAY
      ))
...
get_driver_names_with_retry <- function(season, max_retries = MAX_RETRIES) {
  ...
  for (attempt in 1:max_retries) {
    ...
    if (is.null(drivers) || !is.data.frame(drivers) || nrow(drivers) == 0) {
      ...
      cat(sprintf(
        "  Attempt %d failed for season %d drivers, retrying in %d seconds...\n",
        attempt,
        season,
        RETRY_DELAY
      ))
...
get_constructor_names_with_retry <- function(
  ...
  for (attempt in 1:max_retries) {
    ...
    if (
      is.null(constructors) ||
        !is.data.frame(constructors) ||
        nrow(constructors) == 0
    ) {
      if (attempt == max_retries) {
        return(NULL)
      ...
      cat(sprintf(
        "  Attempt %d failed for season %d constructors, retrying in %d seconds...\n",
        attempt,
        season,
        RETRY_DELAY
      ))
...
calculate_season_points <- function(season) {
  ...
  cat(sprintf(
    "  ✓ Successfully processed %d of %d races for season %d\n",
    successful_races,
    length(races),
    season
  ))
  ...
  race_points <- combined_results %>%
    ...
    mutate(
      ...
      constructor_name = ifelse(
        is.na(constructor_name),
        constructor_id,
        constructor_name
      )
    ) %>%
...
main <- function() {
  ...
  for (season in START_YEAR:END_YEAR) {
    ...
    if (!is.null(season_race_points)) {
      ...
      season_cumulative_points <- calculate_cumulative_points(
        season_race_points
      )
      if (!is.null(season_cumulative_points)) {
        all_cumulative_points[[as.character(
          season
  ...
  if (length(all_race_points) == 0) {
    stop(
      "No data could be processed for any season. Please check your internet connection and API availability."
    )
  }
  ...
  cumulative_points_file <- file.path(
    OUTPUT_DIR,
